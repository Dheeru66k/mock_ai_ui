...
def style_fig(fig, height=300):
    ...
    fig.update_layout(
        ...
        font=dict(color=t["font_color"], family="Plus Jakarta Sans", size=11),
...
#  LOGO & ICON SVGs
# ─────────────────────────────────────────────
def _svg_to_img(svg_str: str, size: int) -> str:
    """Encode an SVG string as a base64 data-URI <img> tag.
    Streamlit strips raw <svg> tags; <img src='data:...'> always renders."""
    encoded = _b64.b64encode(svg_str.strip().encode("utf-8")).decode("utf-8")
    return f'<img src="data:image/svg+xml;base64,{encoded}" width="{size}" height="{size}" style="display:block;">'

def platform_logo_svg(size=72):
    """Hexagonal neural-network platform logo — returned as a safe <img> data URI."""
    a, g = "#00D98A", "#FFB020"
...
def get_uc_logo_svg(uc_type, size=44):
    """Returns a base64 <img> icon for the given use case type.
    Uses hardcoded hex colors (CSS vars don't work in data URIs)."""
    dark = st.session_state.get("dark_mode", True)
    a  = "#00D98A" if dark else "#00A86B"   # accent / emerald
    g  = "#FFB020" if dark else "#D4890A"   # accent2 / amber
    ...
    icons = {
        ...
        "Chatbot": f"""
          <path d="M4,8 H30 Q34,8 34,12 V22 Q34,26 30,26 H18 L10,33 V26 H4 Q4,26 4,22 V12 Q4,8 8,8 Z"
                fill="{a}" fill-opacity="0.1" stroke="{a}" stroke-width="2" stroke-linejoin="round"/>
          <circle cx="13" cy="17" r="2.2" fill="{g}"/>
          <circle cx="22" cy="17" r="2.2" fill="{g}"/>
          <path d="M27,23 H40 Q42,23 42,25 V32 Q42,34 40,34 H35 L30,38 V34 H27 Q25,34 25,32 V25 Q25,23 27,23 Z"
                fill="{a}" fill-opacity="0.2" stroke="{a}" stroke-width="1.5"/>""",

        "Code Generation": f"""
          <polyline points="16,12 6,22 16,32"  fill="none" stroke="{a}"
                    stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"/>
          <polyline points="28,12 38,22 28,32" fill="none" stroke="{a}"
                    stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"/>
          <line x1="25" y1="9" x2="19" y2="35" stroke="{g}"
    ...
    inner = icons.get(uc_type, icons["Other"])
...
def render_login():
    ...
    # Full-page decorative background rings (subtle)
    st.markdown("""
    <style>
    .login-bg {
    ...
    with col_c:
        ...
        with st.form("login_form"):
            st.markdown('<div style="font-size:0.72rem;font-weight:700;color:var(--text2);letter-spacing:0.08em;text-transform:uppercase;margin-bottom:0.3rem;">Account</div>', unsafe_allow_html=True)
            email = st.selectbox(
                "Account",
            ...
            st.markdown('<div style="font-size:0.72rem;font-weight:700;color:var(--text2);letter-spacing:0.08em;text-transform:uppercase;margin-bottom:0.3rem;margin-top:0.8rem;">Password</div>', unsafe_allow_html=True)
            pwd = st.text_input("Password", type="password", placeholder="demo  or  admin",
                                label_visibility="collapsed")
            ...
            if submitted:
                ...
                if pwd == user_data.get("password", ""):
                    ...
                    st.error("Incorrect password. Try: demo (owners / AI team) or admin")

        st.markdown('</div>', unsafe_allow_html=True)

        # Hint + theme toggle
        st.markdown("""
        <div style="text-align:center;margin-top:1rem;font-size:0.78rem;color:var(--text2);">
            Use <code style="background:var(--surface2);padding:1px 6px;border-radius:4px;">demo</code>
            for most accounts &nbsp;·&nbsp;
            <code style="background:var(--surface2);padding:1px 6px;border-radius:4px;">admin</code>
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        <div style="display:flex;align-items:center;gap:0.75rem;
            ...
            <div>
                <div style="font-weight:600;font-size:0.86rem;color:var(--text);">{user['name']}</div>
                <div style="font-size:0.72rem;color:var(--text2);">{role} · {user['unit']}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        def nav_btn(icon, label, page, badge=None):
            ...
            st.markdown(f"""
            <div class="nav-item {'active' if active else ''}" onclick="" style="{'background:rgba(0,217,138,0.10);color:var(--accent);' if active else ''}">
...
def render_dashboard():
    ...
    with col_left:
        ...
        fig = px.area(df_trend, x="Date", y="API Calls",
                      ...
                      color_discrete_sequence=["#00D98A"])
        fig.update_traces(fill="tozeroy", line_width=2,
                          fillcolor="rgba(0,217,138,0.08)")
        style_fig(fig, 280)
    ...
    with col_right:
        ...
        fig2 = go.Figure(go.Pie(
            ...
            marker=dict(colors=["#00D98A","#FFB020","#00D98A","#F39C12","#E74C3C","#34D399","#60A5FA","#A78BFA"]),
        ))
        fig2.update_layout(
            title="Model Distribution",
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color=chart_theme()["font_color"], family="Plus Jakarta Sans"),
    ...
    for uc in sorted_ucs[:6]:
        with st.container():
            ...
            with c1:
                mini_icon = get_uc_logo_svg(uc["type"], size=28)
                st.markdown(f"""
                <div style="display:flex;align-items:center;gap:0.65rem;">
                    <div style="width:32px;height:32px;flex-shrink:0;border-radius:8px;
                                background:rgba(0,217,138,0.08);border:1px solid rgba(0,217,138,0.18);
                                display:flex;align-items:center;justify-content:center;">
                        {mini_icon}
                    </div>
                    <div>
                        <div style="font-weight:600;color:var(--text);font-size:0.88rem;">{uc['name']}</div>
                        <div style="font-size:0.73rem;color:var(--accent);font-weight:500;">{uc['type']} · {uc['business_unit']}</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            with c2:
    ...
    for i in range(0, len(filtered), 2):
        ...
        for col, idx in zip([col_a, col_b], [i, i+1]):
            ...
            with col:
                ...
                icon_svg = get_uc_logo_svg(uc["type"], size=40)
                st.markdown(f"""
                <div class="uc-card">
                    <div style="display:flex;align-items:flex-start;gap:0.9rem;margin-bottom:0.6rem;">
                        <div style="width:44px;height:44px;flex-shrink:0;border-radius:11px;
                                    background:linear-gradient(135deg,rgba(0,217,138,0.1),rgba(255,176,32,0.06));
                        ...
                        <div style="flex:1;min-width:0;">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                                <div class="uc-title" style="margin-bottom:0.15rem;">{uc['name']}</div>
                                {status_badge(uc['status'])}
                            </div>
                            <div style="font-size:0.75rem;color:var(--accent);font-weight:600;
                                        letter-spacing:0.03em;text-transform:uppercase;">{uc['type']}</div>
                        </div>
                    </div>
                    <div class="uc-meta" style="margin-bottom:0.7rem;line-height:1.55;">{uc['description'][:100]}{'...' if len(uc['description'])>100 else ''}</div>
                    <div style="display:flex;gap:1rem;margin-bottom:0.55rem;flex-wrap:wrap;">
                    ...
                    <div style="display:flex;gap:1rem;flex-wrap:wrap;">
                        ...
                        <span class="uc-meta">⏱ {last_str}</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                # Action buttons
...
def render_uc_detail(uc_id):
    ...
    icon_svg = get_uc_logo_svg(uc["type"], size=48)
    st.markdown(f"""
    <div style="display:flex;align-items:center;gap:1rem;margin-bottom:0.3rem;">
        <div style="width:56px;height:56px;flex-shrink:0;border-radius:14px;
                    background:linear-gradient(135deg,rgba(0,217,138,0.12),rgba(255,176,32,0.07));
                    border:1px solid rgba(0,217,138,0.25);
                    display:flex;align-items:center;justify-content:center;">
            {icon_svg}
        </div>
        <div>
            <div class="section-header" style="margin-bottom:0.1rem;">
                {uc['name']} &nbsp;{status_badge(uc['status'])}
            </div>
            <div style="font-size:0.78rem;color:var(--accent);font-weight:600;
                        letter-spacing:0.04em;text-transform:uppercase;">{uc['type']}</div>
        </div>
    </div>
    <div class="section-sub" style="padding-left:4.5rem;">{uc['description']}</div>
    """, unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        with sub_tabs[2]:
            ...
            fig3.add_trace(go.Scatter(x=df3["Date"], y=df3["Success Rate (%)"],
                                       name="Success Rate", line=dict(color="#00D98A", width=2)), secondary_y=False)
            fig3.add_trace(go.Scatter(x=df3["Date"], y=df3["Response Time (ms)"],
                                       name="Response Time", line=dict(color="#FFB020", width=2)), secondary_y=True)
            t = chart_theme()
            fig3.update_layout(paper_bgcolor=t["paper_bg"], plot_bgcolor=t["plot_bg"],
                               font=dict(color=t["font_color"], family="Plus Jakarta Sans"),
...
def render_models():
    ...
    with tabs[1]:
        ...
        fill_map = {"#00D98A": "rgba(0,217,138,0.12)", "#FFB020": "rgba(255,176,32,0.12)"}
        fig = go.Figure()
        for scores, name, color in [(sa, model_a["name"], "#00D98A"), (sb, model_b["name"], "#FFB020")]:
            fig.add_trace(go.Scatterpolar(
                r=scores + [scores[0]], theta=categories + [categories[0]],
                fill="toself", name=name,
                line=dict(color=color, width=2),
                fillcolor=fill_map.get(color, "rgba(0,217,138,0.12)"),
            ))
        t = chart_theme()
        fig.update_layout(
    ...
    with tabs[2]:
        ...
        fig = px.bar(df_mod, x="API Calls", y="Model", orientation="h",
                     ...
                     color="API Calls", color_continuous_scale=["#00D98A","#FFB020"])
        style_fig(fig, 320)
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
...
def render_analytics():
    ...
    with tabs[1]:
        ...
        fig = px.bar(df_bar, x="Use Case", y="API Calls", color="Business Unit",
                     ...
                     color_discrete_sequence=["#00D98A","#FFB020","#00D98A","#F39C12"])
    ...
    with tabs[2]:
        ...
        fig = go.Figure(go.Heatmap(
            ...
            colorscale=[[0,"rgba(0,217,138,0.04)"],[0.5,"rgba(0,217,138,0.45)"],[1,"#00D98A"]],
...
def render_cost_tracking():
    ...
    with tabs[0]:
        ...
        fig = px.bar(df_uc, x="Use Case", y="Monthly Cost ($)", color="Business Unit",
                     ...
                     color_discrete_sequence=["#00D98A","#FFB020","#00D98A","#F39C12","#E74C3C"])
        fig.update_layout(xaxis_tickangle=-30)
        style_fig(fig, 300)
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    ...
    with tabs[1]:
        ...
        fig2 = px.pie(df_model, names="Model", values="Cost ($)", title="Cost Distribution by Model",
                      color_discrete_sequence=["#00D98A","#FFB020","#00D98A","#F39C12","#E74C3C","#34D399","#60A5FA","#A78BFA"])
    ...
    with tabs[2]:
        ...
        fig3 = px.bar(df_unit, x="Business Unit", y="Cost ($)", title="Monthly Cost by Business Unit",
                      color="Cost ($)", color_continuous_scale=["#00D98A","#FFB020"])
        style_fig(fig3, 300)
        st.plotly_chart(fig3, use_container_width=True, config={"displayModeBar": False})
    ...
    with tabs[3]:
        ...
        fig4.add_trace(go.Bar(x=df_trend["Date"], y=df_trend["Daily Cost ($)"],
                               name="Daily Cost", marker_color="rgba(0,217,138,0.5)"), secondary_y=False)
        fig4.add_trace(go.Scatter(x=df_trend["Date"], y=df_trend["Cumulative ($)"],
                                   name="Cumulative", line=dict(color="#FFB020", width=2)), secondary_y=True)
        t = chart_theme()
...
def render_performance():
    ...
    with tabs[0]:
        ...
        fig = px.bar(df_resp, y="Use Case", x=["Avg Response (ms)","P95 (ms)"],
                     ...
                     color_discrete_sequence=["#00D98A","#FFB020"])
        style_fig(fig, 380)
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    ...
    with tabs[1]:
        ...
        fig2 = px.bar(df_succ, x="Success Rate (%)", y="Use Case", orientation="h",
                      ...
                      color_continuous_scale=["#E74C3C","#F39C12","#00D98A"],
                      range_color=[88, 100])
        style_fig(fig2, 380)
        st.plotly_chart(fig2, use_container_width=True, config={"displayModeBar": False})
    ...
    with tabs[2]:
        ...
        fig3 = px.pie(names=error_types, values=error_counts, title="Error Type Distribution",
                      hole=0.5, color_discrete_sequence=["#E74C3C","#F39C12","#00D98A","#FFB020","#00D98A","#34D399"])
...
def render_activity_log():
    ...
    with tabs[0]:
        ...
        activities = []
        actions_pool = [
            ("API Call", "Successful inference", "#00D98A"),
            ("API Call", "Response timeout", "#E74C3C"),
            ("Config Change", "Parameters updated", "#F39C12"),
            ("Access Event", "New user granted access", "#00D98A"),
            ("Model Change", "Model switched", "#FFB020"),
            ("Status Change", "Use case activated", "#00D98A"),
        ]
        np.random.seed(1)
...
def render_feedback():
    ...
    with fc_l:
        ...
        fig = px.bar(x=rating_dist.index, y=rating_dist.values,
                     ...
                     color_continuous_scale=["#E74C3C","#F39C12","#F39C12","#00D98A","#00D98A"])
        style_fig(fig, 240)
        st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
    with fc_r:
        ...
        fig2 = px.pie(names=sent_dist.index, values=sent_dist.values,
                      ...
                      color_discrete_sequence=["#00D98A","#F39C12","#E74C3C"])
